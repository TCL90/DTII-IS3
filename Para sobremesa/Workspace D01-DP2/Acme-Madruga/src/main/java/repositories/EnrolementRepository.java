...
public interface EnrolementRepository extends JpaRepository<Enrolement, Integer> {
	...
	@Query("select e1 from Member m join m.enrolements e1 where m.id=?1 and e1.brotherhood=?2")
	public Collection<Enrolement> existEnrolement(Integer memberId, Integer brotherhoodId);
